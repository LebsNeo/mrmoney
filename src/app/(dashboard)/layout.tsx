...
const navItems = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5M9 11.25h.008v.008H9v-.008zm3 0h.008v.008H12v-.008zm3 0h.008v.008H15v-.008zm-6 3h.008v.008H9v-.008zm3 0h.008v.008H12v-.008zm3 0h.008v.008H15v-.008z" />
      </svg>
    ),
  },
  {
...
const CHANNEL_MANAGER_PATHS = ["/ota", "/ota-payouts", "/settings/ota-channels", "/import/ical"];

const channelManagerItems = [
  {
    href: "/ota",
    label: "OTA Reconcile",
    icon: (
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const isChannelManagerActive = CHANNEL_MANAGER_PATHS.some(p => pathname.startsWith(p));
  const [channelManagerOpen, setChannelManagerOpen] = useState(isChannelManagerActive);

  useEffect(() => {
    if (isChannelManagerActive) setChannelManagerOpen(true);
    ...
    <div className="min-h-screen flex">
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          <div className="pt-0.5">
            ...
            >
              ...
              {/* Channel icon */}
              <span className={cn("transition-colors", isChannelManagerActive ? "text-sky-400" : "text-gray-600")}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8.288 15.038a5.25 5.25 0 017.424 0M5.106 11.856c3.807-3.808 9.98-3.808 13.788 0M1.924 8.674c5.565-5.565 14.587-5.565 20.152 0M12.53 18.22l-.53.53-.53-.53a.75.75 0 011.06 0z" />
                </svg>
              </span>
              <span className="flex-1 text-left">Channel Manager</span>
              {/* Chevron */}
              <svg className={cn("w-3.5 h-3.5 transition-transform duration-200", channelManagerOpen ? "rotate-180" : "")}
              ...
              <div className="mt-0.5 ml-4 pl-3 border-l border-gray-800 space-y-0.5">
                {channelManagerItems.map((sub) => {
                  ...
                  return (
                    ...
                    >
                      <span className={cn("shrink-0", isActive ? "text-sky-400" : "text-gray-600")}>
                        {sub.icon}
                      </span>
                      {sub.label}
                    </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="glass-topbar px-4 lg:px-6 py-3.5 flex items-center gap-4 sticky top-0 z-20">
          ...
          <button
            onClick={() => signOut({ callbackUrl: "/login" })}
            className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-gray-400 hover:text-white hover:bg-gray-800 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" />
            </svg>
